...
public class clearacceptedplayers implements Args {
	private BungeeQuarantine plugin;

	public clearacceptedplayers(BungeeQuarantine pl) {
		plugin = pl;
	}
		...
		if (args.size() == 0) {
			...
			if (plugin.playerDataBase.checkIfTableExists(server) == SQLStates.TRUE) {
				new ConfirmableCommand(plugin, sender,
						"rules {server} clearacceptedplayers confirm".replace("{server}", server),
						plugin.config.ConfiramableCommandTimeout, TimeUnit.SECONDS);
				sender.sendMessage(plugin.msg(plugin.messages.CONFIRMCLEARPLAYERS.replace("{server}", server)));
				return true;
			} else {
				sender.sendMessage(
			...
			if (args.pollFirst().equalsIgnoreCase("confirm")) {
				if (plugin.playerDataBase.checkIfTableExists(server) == SQLStates.TRUE) {
					plugin.playerDataBase.setAllUnaccepted(server);
					sender.sendMessage(plugin.msg(plugin.messages.SETALLPLAYERSUNACCEPTED));
					return true;
				} else {
					sender.sendMessage(
							plugin.msg(plugin.messages.ACCEPTEDPLAYERSERVERNOTEXISTS.replace("{server}", server)));
