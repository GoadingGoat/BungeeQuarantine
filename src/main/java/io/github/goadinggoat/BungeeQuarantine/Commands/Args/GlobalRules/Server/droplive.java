...
public class droplive implements Args {

	private BungeeQuarantine plugin;

	public droplive(BungeeQuarantine pl) {
		plugin = pl;
	}
	...
	public boolean execute(CommandSender sender, String server, String cmd, LinkedList<String> args) {
		if (args.size() == 0) {
			if (RulesManager.serverRules.containsKey(server)) {
				new ConfirmableCommand(plugin, sender, "rules {server} droplive confirm".replace("{server}", server),
						plugin.config.ConfiramableCommandTimeout, TimeUnit.SECONDS);
				sender.sendMessage(plugin.msg(plugin.messages.CONFIRMDROP.replace("{server}", server)));
				return true;
			} else {
			...
			if (args.pollFirst().equalsIgnoreCase("confirm")) {
				if (RulesManager.serverRules.containsKey(server)) {
					RulesManager.drop(server);
					RulesManager.serverRules.remove(server);
					sender.sendMessage(plugin.msg(plugin.messages.DROPPEDLIVERULES.replace("{server}", server)));
					return true;
				} else {
					sender.sendMessage(plugin.msg(plugin.messages.LIVERULESNOTEXIST));
					return true;
